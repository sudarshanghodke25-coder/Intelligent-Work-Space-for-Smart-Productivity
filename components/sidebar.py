"""
Sidebar — Left navigation menu with branding, menu items, tools, and profile card.
"""
...
class SidebarMenuItem(ctk.CTkFrame):
    """Individual clickable sidebar menu item."""

    def __init__(self, parent, icon: str, label: str, is_active: bool = False,
                 on_click=None, **kwargs):
        super().__init__(parent, fg_color="transparent", height=Dims.MENU_ITEM_HEIGHT,
        ...
        self._icon_label = ctk.CTkLabel(
            self._container, text=icon, font=("Segoe UI", 15),
        ...
        self._text_label = ctk.CTkLabel(
            ...
            font=Fonts.MENU_ITEM, text_color=Colors.TEXT_SECONDARY,
    ...
    def set_active(self, active: bool):
        ...
        if active:
            ...
            self._container.configure(fg_color="transparent")
            self._text_label.configure(
                text_color=Colors.TEXT_SECONDARY, font=Fonts.MENU_ITEM
            )
            self._icon_label.configure(text_color=Colors.TEXT_SECONDARY)


class Sidebar(ctk.CTkFrame):
    """Full-height left sidebar with nav menu, tools, and profile."""
    ...
    def _build(self):
        ...
        ctk.CTkLabel(
            ...
            anchor="w", fg_color="transparent"
        ).pack(fill="x", padx=Dims.SIDEBAR_PAD_X + 4, pady=(0, 4))

        for icon, label in MENU_ITEMS:
            item = SidebarMenuItem(
                self, icon=icon, label=label,
                is_active=(label == "Dashboard"),
        ...
        ctk.CTkLabel(
            ...
            font=Fonts.CAPTION, text_color=Colors.TEXT_DIM,
            anchor="w", fg_color="transparent"
        ).pack(fill="x", padx=Dims.SIDEBAR_PAD_X + 4, pady=(16, 4))
    ...
    def _on_menu_click(self, page_name: str):
        """Handle menu item click — update active state and navigate."""
    ...
    def set_active(self, page_name: str):
        """Programmatically set the active menu item."""
