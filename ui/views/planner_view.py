...
class PlannerView(ctk.CTkFrame):
    ...
    def _load_roadmaps_list(self):
        for widget in self.roadmap_list_canvas.winfo_children():
            widget.destroy()
        ...
        if not plans:
            ctk.CTkLabel(self.roadmap_list_canvas, text="No plans created yet.\nGenerate your first roadmap\nwith Aurex AI.", font=Fonts.BODY, text_color=Colors.TEXT_MUTED, justify="center").pack(pady=40)
            return
            
        for p in plans:
            card = ctk.CTkFrame(
                self.roadmap_list_canvas,
                fg_color=Colors.GLASS_FILL_LIGHT if p["id"] != self.current_plan_id else Colors.ACCENT_SUBTLE,
                corner_radius=12, border_width=1,
                border_color=Colors.GLASS_BORDER if p["id"] != self.current_plan_id else Colors.ACCENT_MUTED
            )
            ...
            ctk.CTkLabel(inner, text=p["title"], font=Fonts.BODY_BOLD, text_color=Colors.TEXT_PRIMARY, anchor="w").pack(fill="x")
            
            meta_frame = ctk.CTkFrame(inner, fg_color="transparent")
            meta_frame.pack(fill="x", pady=(5, 0))
            
            ctk.CTkLabel(meta_frame, text=p["status"], font=Fonts.CAPTION, text_color=Colors.ACCENT_GLOW).pack(side="left")
    ...
    def _show_empty_state(self):
        for widget in self.center_canvas.winfo_children():
            widget.destroy()
        ...
        for i, (ex, ptype) in enumerate(examples):
            btn = ctk.CTkButton(
                ...
                fg_color=Colors.GLASS_FILL_LIGHT, hover_color=Colors.ACCENT_SUBTLE,
                border_width=1, border_color=Colors.GLASS_BORDER, corner_radius=12,
    ...
    def _render_workspace(self, plan):
        ...
        # Header
        header_frame = ctk.CTkFrame(self.center_canvas, fg_color="transparent")
        header_frame.pack(fill="x", pady=(0, 10))
        
        ctk.CTkLabel(header_frame, text=plan["title"], font=Fonts.TITLE, text_color=Colors.TEXT_PRIMARY, anchor="w", wraplength=600).pack(side="left", fill="x", expand=True)
        
        # Actions
        actions_frame = ctk.CTkFrame(header_frame, fg_color="transparent")
        actions_frame.pack(side="right")
        ctk.CTkButton(actions_frame, text="🗑 Delete", font=Fonts.SMALL_BOLD, fg_color="transparent", text_color="#E74C3C", hover_color=Colors.GLASS_FILL_LIGHT, width=60, command=lambda: self._delete_plan(plan["id"])).pack(side="right", padx=5)
        
        # --- PLAN SNAPSHOT ---
        snapshot_frame = ctk.CTkFrame(self.center_canvas, fg_color="transparent")
        snapshot_frame.pack(fill="x", pady=(0, 20))
        
        badges = [
            (plan['plan_type'], Colors.GLASS_FILL_LIGHT),
            (plan['status'], Colors.GLASS_FILL_LIGHT),
            (insights.get("difficulty", "Moderate"), Colors.ACCENT_SUBTLE),
            (insights.get("duration", "N/A"), Colors.GLASS_FILL_LIGHT),
            (insights.get("daily_time", "N/A"), Colors.GLASS_FILL_LIGHT),
            (f"Success Prob: {insights.get('success_probability', 'N/A')}", Colors.GLASS_FILL_LIGHT),
            (f"{len(timeline)} Milestones", Colors.GLASS_FILL_LIGHT),
            (f"{len(resources)} Resources", Colors.GLASS_FILL_LIGHT)
        ]
        ...
        if desc:
            ctk.CTkLabel(self.center_canvas, text=desc, font=Fonts.BODY, text_color=Colors.TEXT_SECONDARY, anchor="w", justify="left", wraplength=700).pack(fill="x", pady=(0, 20))
            
        # Aurex Recommendation
        tips = insights.get("tips", [])
        if tips:
            rec_frame = ctk.CTkFrame(self.center_canvas, fg_color=Colors.ACCENT_SUBTLE, corner_radius=12, border_width=1, border_color=Colors.ACCENT_MUTED)
            ...
            for tip in tips:
                ctk.CTkLabel(inner_rec, text=f"• {tip}", font=Fonts.BODY, text_color=Colors.TEXT_PRIMARY, justify="left", wraplength=650, anchor="w").pack(fill="x", pady=2)
        ...
        # Timeline
        if timeline:
            ctk.CTkLabel(self.center_canvas, text="TIMELINE OVERVIEW", font=Fonts.SMALL_BOLD, text_color=Colors.ACCENT_GLOW, anchor="w").pack(fill="x", pady=(0, 10))
            tl_frame = ctk.CTkFrame(self.center_canvas, fg_color=Colors.GLASS_FILL_LIGHT, corner_radius=12)
            tl_frame.pack(fill="x", pady=(0, 30))
            
            inner_tl = ctk.CTkFrame(tl_frame, fg_color="transparent")
            inner_tl.pack(fill="x", padx=15, pady=15)
            ...
            for i, phase in enumerate(timeline):
                ...
                if i < len(timeline) - 1:
                    ctk.CTkLabel(inner_tl, text="➞", font=Fonts.BODY, text_color=Colors.ACCENT_MUTED).pack(side="left", padx=(0, 10))

        # Steps
        if steps:
            ctk.CTkLabel(self.center_canvas, text="STRUCTURED STEPS", font=Fonts.SMALL_BOLD, text_color=Colors.ACCENT_GLOW, anchor="w").pack(fill="x", pady=(0, 10))
            
            for step in steps:
                step_card = ctk.CTkFrame(self.center_canvas, fg_color=Colors.GLASS_FILL_LIGHT, corner_radius=12, border_width=1, border_color=Colors.GLASS_BORDER)
                ...
                ctk.CTkLabel(header, text=step.get("title", ""), font=Fonts.BODY_BOLD, text_color=Colors.TEXT_PRIMARY).pack(side="left")
                
                # Details
                details_frame = ctk.CTkFrame(step_card, fg_color="transparent")
                details_frame.pack(fill="x", padx=(45, 15), pady=(0, 15))
                ctk.CTkLabel(details_frame, text=step.get("details", ""), font=Fonts.BODY, text_color=Colors.TEXT_SECONDARY, justify="left", wraplength=600).pack(anchor="w")
    ...
    def _create_expandable_panel(self, title, content_builder):
        panel = ctk.CTkFrame(self.center_canvas, fg_color=Colors.GLASS_FILL_LIGHT, corner_radius=12, border_width=1, border_color=Colors.GLASS_BORDER)
    ...
    def _build_bullet_list(self, frame, items, color=Colors.TEXT_SECONDARY):
        for item in items:
            ctk.CTkLabel(frame, text=f"• {item}", font=Fonts.BODY, text_color=color, justify="left", wraplength=650, anchor="w").pack(fill="x", pady=2)
    ...
    def _build_checkbox_list(self, frame, items):
        for item in items:
            ...
            ctk.CTkLabel(row, text=item, font=Fonts.BODY, text_color=Colors.TEXT_SECONDARY, justify="left", wraplength=620, anchor="w").pack(side="left", fill="x")
    ...
    def _build_tools_list(self, frame, tools):
        for tool in tools:
            ...
            btn = ctk.CTkButton(
                ...
                fg_color="transparent", hover_color=Colors.GLASS_FILL_HOVER,
                border_width=1, border_color=Colors.GLASS_BORDER, text_color=Colors.ACCENT_PRIMARY,
    ...
    def _show_empty_resources(self):
        for widget in self.resources_canvas.winfo_children():
            widget.destroy()
        ctk.CTkLabel(
    ...
    def _render_resources(self, resources_json):
        ...
        for res in resources:
            card = ctk.CTkFrame(self.resources_canvas, fg_color=Colors.GLASS_FILL_LIGHT, corner_radius=12, border_width=1, border_color=Colors.GLASS_BORDER)
            card.pack(fill="x", pady=6)
            
            inner = ctk.CTkFrame(card, fg_color="transparent")
            inner.pack(fill="both", expand=True, padx=12, pady=12)
            
            ctk.CTkLabel(inner, text=res.get("title", "Resource"), font=Fonts.BODY_BOLD, text_color=Colors.TEXT_PRIMARY, wraplength=250, justify="left", anchor="w").pack(fill="x")
            ctk.CTkLabel(inner, text=res.get("type", "Link"), font=Fonts.CAPTION, text_color=Colors.ACCENT_MUTED, anchor="w").pack(fill="x", pady=(2, 8))
            
            btn = ctk.CTkButton(
                inner, text="Open Browser ↗", font=Fonts.SMALL_BOLD, height=24,
                fg_color="transparent", hover_color=Colors.ACCENT_SUBTLE, border_width=1, border_color=Colors.ACCENT_PRIMARY, text_color=Colors.ACCENT_GLOW,
    ...
    def _build_bottom_panel(self):
        self.bottom_frame = ctk.CTkFrame(self, fg_color=Colors.GLASS_FILL, corner_radius=20, border_width=1, border_color=Colors.GLASS_BORDER, height=120)
    ...
    def _trigger_generation(self):
        ...
        for widget in self.center_canvas.winfo_children():
            widget.destroy()
        ctk.CTkLabel(self.center_canvas, text="Aurex AI is building your roadmap...", font=Fonts.HEADING, text_color=Colors.ACCENT_GLOW).pack(pady=100)
    ...
    def _on_roadmap_generated(self, payload):
        ...
        if not payload.get("success"):
            for widget in self.center_canvas.winfo_children(): widget.destroy()
            ctk.CTkLabel(self.center_canvas, text=f"Failed to generate: {payload.get('error')}", text_color=Colors.CHART_RED).pack(pady=100)
            return
